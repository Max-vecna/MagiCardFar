import { saveData, getData } from './db-local.js';
import { renderInventoryForForm } from './manager-item.js';
...
async function createSelectedElement(data, type) {
    ...
    if (type === 'magic') {
        ...
        previewType = data.type === 'habilidade' ? 'skill' : 'spell';
    } else if (type === 'skill') {
        ...
        previewType = data.cardType === 'creature' ? 'creature' : 'character';
    } else if (type === 'item') {
    ...
    if (type === 'relationship' || type === 'magic' || type === 'skill' || type === 'attack') {
        ...
        itemElement.innerHTML = `
            ${renderCompactCardPreview(data, previewType)}
...
export async function saveCharacterCard(cardForm) {
    ...
    delete cardData.receiverIconType;
    delete cardData.receiverIconMode;
    delete cardData.receiverIconTarget;
    delete cardData.receiverIconFree;
    delete cardData.receiverIcon;
    delete cardData.iconReceiverType;
    delete cardData.iconReceiverMode;
    delete cardData.iconReceiverTarget;
    delete cardData.iconReceiverFree;
