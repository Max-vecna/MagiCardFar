...
async function createMiniCards()
{
    ...
    if (effects && effects.length > 0) {
        const effectCardsHtml = effects.map((eff) => {
            return `
                <div class="related-spell-grid-item" data-id="${eff.id}" data-type="effect" style="margin-right: -15px;">
                    ${renderCompactCardPreview(eff, eff.type === 'habilidade' ? 'skill' : (eff.type === 'ataque' ? 'attack' : 'spell'))}
                </div>
            `;
        });
    ...
    if (characters.length > 0) {
        const charactersCardsHtml = characters.map((character) => {
            return `
                <div class="related-character-grid-item" data-id="${character.id}" data-type="character" style="margin-right: -15px;">
                    ${renderCompactCardPreview(character, character.cardType === 'creature' ? 'creature' : 'character')}
                </div>
            `;
        });
        ...
        const itemsCardsHtml = items.map((item) => {
            return `
                <div class="related-item-grid-item" data-id="${item.id}" data-type="item" style="margin-right: -15px;">
                    ${renderCompactCardPreview(item, 'item')}
                </div>
            `;
        });
   ...
   let grid = await createMiniCards();
    ...
    modal.innerHTML = `
        <div class="bg-gray-900 border-2 border-indigo-800/50 text-white rounded-2xl shadow-2xl w-72 max-w-sm text-center p-6 relative" style="overflow: hidden;">
            ...
            <button id="abort-progress-btn" class="mt-4 text-xs text-red-400 hover:text-red-300 hidden">Cancelar OperaÃ§Ã£o</button>
        </div>
    `;
...
export function showProgressModal(title = "Processando...", initialPercent = 0) {
    ...
    modal.querySelector('#abort-progress-btn').classList.add('hidden'); // Reset do botÃ£o
...
export async function importDatabase(file, onProgress = () => {}) {
    ...
    try {
        ...
        updateProgress("ImportaÃ§Ã£o concluÃ­da!", 100);
        setTimeout(hideProgressModal, 500);
    } catch(e) {
        hideProgressModal();
        console.error(e);
...
export async function exportImagesAsPng(onProgress = () => {}) {
    ...
    try {
        ...
        localOnProgress("A carregar imagens do grimÃ³rio...");
        ...
        hideProgressModal();
        console.error("Erro na exportaÃ§Ã£o de imagens:", e);
        showTopAlert("Erro na exportaÃ§Ã£o de imagens.", 3000, "error");
    }
}
