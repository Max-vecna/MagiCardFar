import { getData, saveData } from './db-local.js';
...
export async function renderFullCharacterSheet(characterData, isModal, isInPlay, targetContainer, renderOptions = {}) {
    ...
    if (isModal) {
        ({ finalWidth, finalHeight } = shrinkExpandedCardSize({ finalWidth, finalHeight }));
    }
    ...
    const renderCollectionMiniCard = (config, item) => {
        let wrapperClass = 'related-spell-grid-item';
        let previewType = 'spell';

        if (config.type === 'character') {
            wrapperClass = 'related-character-grid-item';
            previewType = item.cardType === 'creature' ? 'creature' : 'character';
            ...
            previewType = item.type === 'habilidade' ? 'skill' : (item.type === 'ataque' ? 'attack' : 'spell');
        }
        ...
        return `
            <div
                ...
                ${renderCompactCardPreview(item, previewType)}
            </div>
        `;
    };
    ...
    const ensureCollectionPanelRendered = async (collectionKey) => {
        ...
        try {
            grid.innerHTML = config.items.map(item => renderCollectionMiniCard(config, item)).join('');
